...
struct Connection : std::enable_shared_from_this<Connection> {
    ...
    void handle_reg(const Packet& p) {
        ...
        LWEVector s_sv_vec    = LWEVector::noise_vector();  // 小秘密向量 ∈ [-η, η]^n
    ...
    void handle_verify(const Packet& p) {
        ...
        auto c1_bar_arr           = p.body["c1_bar"].get<std::vector<long long>>();
        ...
        Logger::print_kv("c1_bar (bits)", (long long)c1_bar_arr.size());
        ...
        // LWE 解密 s：逐 bit 解码 c1[j] - u1·s_server
        LWEVector u1v(LWE_N); u1v.data = u1;
        LWEVector s_sv_vec; s_sv_vec.data = user.s_server;
        long long base_noise = (long long)u1v.dot(s_sv_vec);
        std::vector<int> dec_bits(LWE_MSG_BITS);
        for (int j = 0; j < LWE_MSG_BITS; ++j) {
            long long c1_j = decomp(c1_bar_arr[j]);
            long long val  = ((c1_j - base_noise) % LWE_Q + LWE_Q) % LWE_Q;
            dec_bits[j] = decode_bit(val);
        }
        long long s_recv = bits_to_value(dec_bits);
        ...
        Logger::print_sep();
        Logger::print_kv("base_noise = u1·s_server", base_noise);
        Logger::print_vec("s_server (vec)",           user.s_server);
        Logger::print_kv("s_recv (bit-decoded)",      s_recv);
        ...
        long long u2_s2 = ((long long)u2v.data[0] * s2 % LWE_Q + LWE_Q) % LWE_Q;
        auto v2_bits    = value_to_bits(v2);
        std::vector<long long> c2_arr(LWE_MSG_BITS);
        for (int j = 0; j < LWE_MSG_BITS; ++j) {
            int e_c2_j     = lwe_noise();
            long long c2_j = (u2_s2 + e_c2_j + encode_bit(v2_bits[j]) + LWE_Q) % LWE_Q;
            c2_arr[j]      = comp(c2_j);
        }
